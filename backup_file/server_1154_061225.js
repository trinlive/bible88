...
// Update: Added Interactive HTML Links for Cross-References
...
app.post('/api/get-chapter', async (req, res) => {
    ...
    try {
        ...
        // *** Updated Prompt: สั่งให้สร้าง Link ที่กดได้ (Interactive Links) ***
        const prompt = `
            ...
            1. Translation: Translate to Thai using the literary style of the **Thai New Contemporary Bible (TNCV)** by Biblica.
               ...
               - **STRUCTURE & HEADERS (STRICT)**:
                 ...
                 - **IMPORTANT**: Do NOT use special colors/spans for Divine Names inside headers. Keep header text clean and consistent in color.
               ...
               - **DIVINE NAMES RULES (MANDATORY)**:
                 1. For "Yahweh", "The LORD" (YHWH) -> Must translate EXACTLY as:
                    <span class="font-bold text-amber-700 mx-1">พระยาห์เวห์</span>
            ...
            2. Summary: Provide sidebar commentary in Thai HTML. 
              - **NO HEADER**: Do NOT generate a main title or header for the commentary. Start directly with the analysis paragraphs.
               ...
               - **CROSS-REFERENCES (INTERACTIVE LINKS)**: 
                 - Include relevant scripture references (Cross-references) from other books of the Bible that connect contextually. Format as ดู: [Book] [Chapter]:[Verse].
                 - When citing other scriptures, **generate clickable HTML links** that call the JavaScript function 'openChapterFromSearch'.
                 - **Format**: <a href="#" onclick="openChapterFromSearch('EnglishBookName', ChapterNumber); return false;" class="text-indigo-600 hover:text-indigo-800 hover:underline font-medium cursor-pointer" title="คลิกเพื่ออ่าน">(ดู: ThaiBookName Chapter:Verse)</a>
                 - Example: <a href="#" onclick="openChapterFromSearch('Genesis', 1); return false;" class="text-indigo-600 hover:text-indigo-800 hover:underline font-medium cursor-pointer">(ดู: ปฐมกาล 1:26)</a>
               
               - **SECTION 1: Word Study (CRITICAL)**: 
                 - Title: "🔍 เจาะลึกรากศัพท์" (Word Study)."
                 - Select 1-3 key terms from this chapter in their original language (Hebrew, Aramaic).
