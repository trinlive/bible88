...
function drawMoonSVG(age) {
    ...
    if (age <= synodic/2) { 
        // ข้างขึ้น
        path = `M 50,0 A 50,50 0 1,1 50,100 A ${Math.abs(x)},50 0 1,${x > 0 ? 1 : 0} 50,0`;
    } else { 
        // ข้างแรม
        path = `M 50,0 A 50,50 0 1,0 50,100 A ${Math.abs(x)},50 0 1,${x > 0 ? 0 : 1} 50,0`;
    }
...
function renderMoonWidget(containerId) {
    ...
    const fullPhaseName = getPhaseName(age); // ได้ค่า "🌕 จันทร์เพ็ญ (Full Moon)"
    
    // --- แก้ไขจุดนี้: ตัด Emoji ออก ---
    // 1. split(" (")[0] -> ตัดคำอธิบายภาษาอังกฤษออก เหลือ "🌕 จันทร์เพ็ญ"
    // 2. split(" ").slice(1).join(" ") -> ตัดคำแรก (Emoji) ออก เหลือแค่ "จันทร์เพ็ญ"
